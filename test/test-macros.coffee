...
fmt = (pat, args...) -> 
	inst = new PatternFormatter(pattern: pat, filename: __filename, winston: Winston).formatter()
...
test 'filename', (t) ->
	...
	t.equals fmt("%pkg{name}"), pkgjson.name, 'filename/pkg{name}'
	t.equals fmt("%pkg{name}%pkg{version}"), pkgjson.name + pkgjson.version, 'filename/pkg{name} + pkg{version}'
	t.equals fmt("%pkg{version}"), pkgjson.version, 'filename/pkg{version}'
	git_rev = fmt("%git-rev")
